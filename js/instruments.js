...
// sine-based sub bass that follows the chord roots.

export function createPad() {
  const filter = new Tone.Filter({ frequency: 2200, type: 'lowpass', rolloff: -24 }).toDestination();
...
export function createBass() {
  const filter = new Tone.Filter({ frequency: 500, type: 'lowpass', rolloff: -12 }).toDestination();
