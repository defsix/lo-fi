...
export function createKeys(bus) {
  const filter = new Tone.Filter({ frequency: 1900, type: 'lowpass', rolloff: -24 }).connect(bus);
...
export function createLead(bus) {
  const filter = new Tone.Filter({ frequency: 2600, type: 'lowpass', rolloff: -12 }).connect(bus);
